...
import { addTestflowTestsMigration } from "migrations/add-testflow-tests.migration";
...
@Module({
  ...
  providers: [databaseProvider, addTestflowTestsMigration],
})
class MigrationModule {}

async function run() {
  const app = await NestFactory.createApplicationContext(MigrationModule);
  const migration = app.get(addTestflowTestsMigration);
