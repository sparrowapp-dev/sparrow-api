...
import { UpdatePlanMigration } from "migrations/update-plan.migration";
...
@Module({
  ...
  providers: [databaseProvider, UpdatePlanMigration],
})
class MigrationModule {}

async function run() {
  const app = await NestFactory.createApplicationContext(MigrationModule);
  const migration = app.get(UpdatePlanMigration);
