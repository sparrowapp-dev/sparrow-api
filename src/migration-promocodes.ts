...
import { CreatePromocodesMigration } from "migrations/create-promocodes.migration";
...
@Module({
  ...
  providers: [databaseProvider, CreatePromocodesMigration],
})
class MigrationModule {}

async function run() {
  const app = await NestFactory.createApplicationContext(MigrationModule);
  const migration = app.get(CreatePromocodesMigration);
