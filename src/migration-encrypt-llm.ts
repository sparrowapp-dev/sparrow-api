...
import { EncryptLlmApiKeysMigration  } from "migrations/encrypt-llm.migration";
...
@Module({
  ...
  providers: [databaseProvider, EncryptLlmApiKeysMigration ],
})
class MigrationModule {}

async function run() {
  const app = await NestFactory.createApplicationContext(MigrationModule);
  const migration = app.get(EncryptLlmApiKeysMigration );
