...
import { UpdateWorkspaceTypeMigration } from "migrations/update-workspaceType.migration";
...
@Module({
  ...
  providers: [databaseProvider, UpdateWorkspaceTypeMigration],
})
class MigrationModule {}

async function run() {
  const app = await NestFactory.createApplicationContext(MigrationModule);
  const migration = app.get(UpdateWorkspaceTypeMigration);
