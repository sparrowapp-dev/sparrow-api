...
import { AIlogsEmailIdMigration } from "migrations/ai-emailId.migration";
...
@Module({
  ...
  providers: [databaseProvider, AIlogsEmailIdMigration],
})
class MigrationModule {}

async function run() {
  const app = await NestFactory.createApplicationContext(MigrationModule);
  const migration = app.get(AIlogsEmailIdMigration);
