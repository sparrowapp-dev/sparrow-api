...
import { MockRequestUrlMigration } from "migrations/update-mock-request-url.migration";
...
@Module({
  ...
  providers: [databaseProvider, MockRequestUrlMigration],
})
class MigrationModule {}

async function run() {
  const app = await NestFactory.createApplicationContext(MigrationModule);
  const migration = app.get(MockRequestUrlMigration);
