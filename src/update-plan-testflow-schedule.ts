...
import { UpdatePlanTestflowScheduleMigration } from "migrations/update-plan-testflow-schedule.migration";
...
@Module({
  ...
  providers: [databaseProvider, UpdatePlanTestflowScheduleMigration],
})
class MigrationModule {}

async function run() {
  const app = await NestFactory.createApplicationContext(MigrationModule);
  const migration = app.get(UpdatePlanTestflowScheduleMigration);
