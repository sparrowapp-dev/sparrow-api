...
import { CreateSuperadminMigration } from "migrations/create-superadmin.migration";
...
@Module({
  ...
  providers: [databaseProvider, CreateSuperadminMigration],
})
class MigrationModule {}

async function run() {
  const app = await NestFactory.createApplicationContext(MigrationModule);
  const migration = app.get(CreateSuperadminMigration);
